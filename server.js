...
app.get('/askDBPedia', (req, res) => {
  var q = "select ?s1 as ?c1, (bif:search_excerpt (bif:vector ('MAGGIORE', 'PIAZZA'), ?o1)) as ?c2, ?sc, ?rank, ?g where {{{ select ?s1, (?sc * 3e-1) as ?sc, ?o1, (sql:rnk_scale (<LONG::IRI_RANK> (?s1))) as ?rank, ?g where  { quad map virtrdf:DefaultQuadMap { graph ?g { ?s1 ?s1textp ?o1 . ?o1 bif:contains  '(MAGGIORE AND PIAZZA)'  option (score ?sc)  . } } } order by desc (?sc * 3e-1 + sql:rnk_scale (<LONG::IRI_RANK> (?s1)))  limit 1  offset 0 }}}";
      ...
      if (!error) {
        ...
        mydb.collection("Descrizioni").find({ nome: req.query.val }).toArray(async function (err, result) {
          ...
          if (result.length !== 0) {
            ...
            var d = await f.get('http://localhost:8000/askDBPedia');
