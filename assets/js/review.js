...
function sendReview(){
  if(voto == null){
      ...
      if(update == false){
        $.ajax({
          type: "post",
          url: "insertReview?luogo="+$("#reviewLuogo").text()+"&voto=3&descrizione="+$("#txtReview").val()+"&wr="+profile.getName()+"&rd="+profile.getName(),
...
function checkReview(){
  $.ajax({
    type: "get",
    url: "getReview?luogo="+$("#reviewLuogo").text()+"&wr="+profile.getName()+"&mode=check",
     success: function(result){
       if(result.length != 0){
         update = true;
